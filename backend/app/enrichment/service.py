...
from ..enrichment.llm.anthropic_provider import AnthropicLLMProvider
...
class EnrichmentService:
    ...
    def __init__(self, db_client: AsyncIOMotorClient, config):
    ...
    def _initialize_provider(self):
        ...
        try:
            if self.config.use_anthropic:
                self._provider = AnthropicLLMProvider(llm_config)
    ...
    async def process_hit(self, hit_id: str, brand_id: str) -> Optional[EnrichmentResult]:
        ...
        # Fetch brand details
        try:
            brand = await self.db.brands.find_one({"_id": hit["brand_id"]})
        except Exception:
            brand = await self.db.brands.find_one({"_id": ObjectId(hit["brand_id"])})
...
Evaluate this hit and return your analysis as JSON."""
    ...
    async def _store_result(self, result: EnrichmentResult):
